...
# Claude 投影复用共享门禁模块（与 Codex 投影同一套路径/SHA/catalog/blocked 逻辑）。
# 本脚本是 Claude 投影的唯一 writer；fresh-session smoke 保持 UNVERIFIED。
...
function Get-ClaudeProjectionPlan {
    ...
    return [pscustomobject]@{
        ...
        catalogRelativePath = $codexPlan.catalogRelativePath
        catalogSha256 = $codexPlan.catalogSha256
        sourceRevision = $codexPlan.sourceRevision
        entryPath = $codexPlan.entryPath
        files = $files
        acceptedPrimitiveIds = $codexPlan.acceptedPrimitiveIds
        blockedRecords = $codexPlan.blockedRecords
        entryAliases = $codexPlan.entryAliases
        forbiddenSegments = $codexPlan.forbiddenSegments
    }
}
...
function Test-ClaudeRuntimeProjection {
    ...
    $plan = Get-ClaudeProjectionPlan -Root $Root
    $manifestRelativePath = 'claude-projection-manifest.json'
    ...
    if ($manifest.schema -ne 'feisheng-claude-runtime-projection/v1') {
    ...
    return [ordered]@{
        schema = 'feisheng-claude-runtime-projection-result/v1'
        mode = 'Validate'
        status = 'PASS'
...
try {
    ...
    if ($Mode -eq 'Build') {
        ...
        $plan = Get-ClaudeProjectionPlan -Root $repositoryRootFull
        ...
        $manifest = [ordered]@{
            schema = 'feisheng-claude-runtime-projection/v1'
        ...
        $manifestPath = Join-ContainedPath -Root $outputRootFull -RelativePath 'claude-projection-manifest.json'
        $manifest | ConvertTo-Json -Depth 12 | Set-Content -Encoding UTF8 -LiteralPath $manifestPath
    }

    $result = Test-ClaudeRuntimeProjection -Root $repositoryRootFull -ProjectionRoot $outputRootFull
    if ($Mode -eq 'Build') {
        $result.mode = 'Build'
    }

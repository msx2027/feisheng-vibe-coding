...
| `hotspot-governor` | vibe-coding-skills | product-or-checker | accepted-product | none | `directory` | 17 |
...
重叠能力已归属到唯一 owner；同一能力有多个技能时按下列规则分工。owner 均登记在 `provenance/OWNER-LEDGER.json`。被标记**（已退役）**的成员不在 runtime 集合内、不可调用，其分工规则文案为退役前口径，仅供溯源。

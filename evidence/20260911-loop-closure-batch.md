# 证据：闭环补全批——27 个能力接入，vibe coding 全链路闭环
...
**决策不搬（16 个）及理由**：design-maker、codebase-memory-scout（依赖外部 MCP，未配置即死代码）；
...
- 本机门禁 **13/13**：runtime include 94→**331 文件**；路由绑定 admitted=**36** bound=**36**；NOTICE runtime items=38
- fresh clone 四组合（autocrlf true/false × pwsh/PS5.1）**全部 13/13**
- 共享根重装：**333 文件**（sourceRevision=`8c800d7`，builder Validate 通过）；目录 checker/engineering/product/ui 四组
- Codex `debug prompt-input`：注入总条目 229，包内 38 个 SKILL.md 全部可见（抽查 product-spec-builder/impeccable/

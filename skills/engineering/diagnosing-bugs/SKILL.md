...
**然后问：什么本可以预防这个 bug？** 如果答案涉及 architecture change（没有好 test seam、callers 缠绕、hidden coupling），带着具体细节交给 `codebase-design` skill（本包已接入的深模块设计能力）。这个建议要在 fix 之后提出，不要在之前提出；现在你比开始时知道得更多。

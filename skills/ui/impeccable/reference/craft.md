...
Run /design-brief-builder, passing along whatever feature description the user provided.
...
If the user has already run /design-brief-builder and has a confirmed design brief, skip this step and use the existing brief.
...
### During Build
- Test with real (or realistic) data at every step, not lorem-style placeholder text; intentional static mocks or placeholders are allowed when the feature/backend is outside the request
- Use realistic static or mock data for visual fidelity when the feature/backend is not part of the request; do not wire new business logic, persistence, API calls, submissions, permissions, payments, deletes, or background jobs unless explicitly requested

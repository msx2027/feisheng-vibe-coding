---
name: design-system
description: '仅当用户先明确调用 `feisheng-vibe-coding` 总入口并指定 `design-system`，或由该总入口按指定路由到本 Skill 时使用；不得根据用户自然语言独立触发；Token architecture, component specifications, CSS...'

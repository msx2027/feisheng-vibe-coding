Update brand colors, typography, and style, then sync them into the project design token files.
...
- Generate and validate temporary JSON/CSS outputs before committing either target
- Exit non-zero and preserve or restore both original targets if generation, path validation, compare-and-swap, or either replacement fails
...
- **Treat JSON and CSS as one transaction** - Never accept a run that updates only one token target

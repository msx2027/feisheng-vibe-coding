#!/usr/bin/env node
// [部署副本] 来源 sources/vibe-coding-skills/tools/experience-ledger-core.mjs（快照闭点 2026-09-11）。
// 部署 delta（evidence/20260923-experience-backflow-batch-b.md）：
//   ① 经验记录新增可选 theme 字段（固定枚举 EXPERIENCE_THEMES，未分类=字段缺席）；
//   ② 归档记录同样允许保留 theme；其余与快照逐字一致。
...
// [部署副本新增] 经验主题固定枚举（「疼的部位」）。未分类 = 字段缺席，不设 unknown 值，避免枚举漂移。
...
function assertValidExperienceBase(record, where, archived) {
  ...
  const optional = archived
    ? ["theme"]
    : ["confirmationHistory", "confirmation", "theme"];
  ...
  if (record.theme !== undefined && !EXPERIENCE_THEMES.includes(record.theme)) {
    throw new Error(`${where}: theme 必须是 EXPERIENCE_THEMES 内的固定枚举值`);
  }
  if (archived) {

#!/usr/bin/env node
// [部署副本] 来源 sources/vibe-coding-skills/tools/experience-governance.mjs（快照闭点 2026-09-11）。
// 部署 delta（evidence/20260923-experience-backflow-batch-b.md）：
//   台账写回由整文件重渲染改为 spliceLedgerFence 围栏拼接：只替换
//   ```json vibe-experience-ledger 围栏内的 JSON，围栏外内容（清扫政策围栏、
//   人工登记区等）原样保留——与 experience-recorder.mjs 的写入语义一致。
//   整文件重渲染会静默删除围栏外的政策围栏与人工登记（fs-agent 实账即含这些内容）。
...
// [部署副本新增] 台账围栏拼接：只替换真源围栏内的 JSON，围栏外内容原样保留。
const LEDGER_FENCE_OPEN = "```json vibe-experience-ledger";
const LEDGER_FENCE_CLOSE = "```";
function spliceLedgerFence(originalMarkdown, nextLedger) {
  const openIdx = originalMarkdown.indexOf(LEDGER_FENCE_OPEN);
  if (openIdx === -1) throw new Error("ledger fence open missing");
  const closeIdx = originalMarkdown.indexOf("\n" + LEDGER_FENCE_CLOSE, openIdx + LEDGER_FENCE_OPEN.length);
  if (closeIdx === -1) throw new Error("ledger fence close missing");
  const prefix = originalMarkdown.slice(0, openIdx);
  let suffix = originalMarkdown.slice(closeIdx + 1 + LEDGER_FENCE_CLOSE.length);
  if (suffix !== "" && !suffix.startsWith("\n")) suffix = "\n" + suffix;
  return prefix + serializeLedger(nextLedger) + suffix;
}
...
function adoptL1RegistryAnchor(targetRoot, paths, ledgerContent, adoption, currentRegistryInfo, request) {
  ...
  const transaction = commitTargetTransaction(targetRoot, {
    ...
    operations: [{ file: paths.ledger, content: spliceLedgerFence(ledgerContent, nextLedger), expectedContent: ledgerContent }],
  });
...
export function executeExperienceAction(request) {
  ...
  const operations = [
    ...
    ...(nextConstitution !== constitutionContent
      ...
      : []),
    { file: paths.ledger, content: spliceLedgerFence(ledgerContent, nextLedger), expectedContent: ledgerContent },
  ];
  const transaction = commitTargetTransaction(targetRoot, {
    journalPath: ".vibe-experience-transaction.json",

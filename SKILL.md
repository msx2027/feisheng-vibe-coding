...
2. **目标项目运行件装配（环境启动，先于路由与开发）**：本次会话锚定了某个目标项目时，先检查该项目结构棘轮门禁是否已装配（`tools/check-hotspots.mjs` 在位且 pre-commit 已接线）；缺失则执行本技能目录下 `skills/product/hotspot-governor/tools/install-hotspot-gate.mjs <目标项目根>` 自动装配并输出基线报告，已装配则零改动跳过。装配幂等、不覆盖项目本地适配；owner 已于 2026-09-17 预授权为默认动作，无需逐次请示。装配不属于路由决策，控制面地位不受影响；装配结果如实报告，不得在未跑装配前声明"门禁已生效"。
3. 按控制面文件的 Startup Protocol 执行：先确认当前真相与授权边界，再用控制面自带工具选择唯一主路由，最后按路由加载对应能力的 owner 文件。
4. 项目级请求不得跳过控制面直接作答；内部能力只能返回结果或 finding，不得自行改选主路由。

## 当前阶段

仓库处于闭环后受控运行阶段（2026-09-11）：82 条来源技能已登记，**52 条通过五道门禁进入 runtime**（420 文件）；自然语言触发（D2）与全链路路由（D3）已在宿主实测转绿；GitHub Actions CI 已全绿（verify.ps1 静态门禁 + 发布候选包装配）。三个源项目已归档冷存并删除本体，仓库内 `sources/` 快照为唯一内容真源。宿主 trust、逐技能行为与 Hook fresh-session 冒烟未验证前，相关结论一律保持 `UNVERIFIED`。

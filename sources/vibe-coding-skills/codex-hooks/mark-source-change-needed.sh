...
# Syncs with: hooks/mark-source-change-needed.sh, codex-hooks.json, tools/review-gate.sh, tools/doc-sync-gate.sh
# Codex PostToolUse hook: 合并 source-change review 和 doc-sync 标记，减少每次编辑后的固定进程成本
...
process.stdin.on("end", () => {
  try { process.stdout.write(String(JSON.parse(input)?.tool_input?.file_path || "")); } catch {}
});
' 2>/dev/null)

...
> 规范关系：`execution tier` 决定实施与验证强度，`review profile` 决定审查阶段、逻辑角色/上下文边界和 finding 闭环。二者正交计算，只允许风险信号把任一维度单向升级，不得相互降级。这里的 Reviewer“隔离”不是物理进程隔离；物理只读沙箱仅在用户明确要求时启用。
...
`independent-two-stage` 和 `hazard-review` 必须使用两个 fresh Reviewer 实例；同一个 code-reviewer Agent 可以作为角色真源，但每个实例只接收 `reviewStage=spec` 或 `reviewStage=quality`。这里的两个实例是逻辑审查实例，不要求独立进程或物理沙箱。两个 Reviewer 不继承彼此对话或结论。只有用户明确要求物理隔离时，才使用物理只读启动器。
...
- 没有物理隔离不构成普通任务阻断。Reviewer 启动失败、超时或只返回 `partial` / `review-unavailable` 时，主 Agent继续逻辑审查和验证，并披露审查缺口；不得伪造物理 Review Receipt，也不得把物理隔离当作默认完成条件。

...
Pin an approved exact `shadcn` version in the target project's `package.json`,
commit the matching package-manager lock file, and install dependencies from that
lock file. For initial setup, replace `X.Y.Z` with the reviewed exact version:

以下依赖只有在用户明确同意后才能安装；未获同意时不得执行安装命令。
...
On Windows PowerShell, use the `.cmd` npm launcher explicitly and keep npm offline:
...
The helper validates `node_modules/shadcn/package.json`, rejects a `bin` entry that
escapes that package directory, and executes the contained JavaScript entry with
Node and `shell=False`. Missing lock/package/Node state stops with setup guidance.

...
description: 仅当用户先明确调用 `vibe-coding-skills` 总入口并指定 `design-brief-builder`，或由该总入口按指定路由到本 Skill 时使用；不得根据用户自然语言独立触发。当用户说要确定设计风格、视觉方向，或说'我想要高级感/简洁/现代'这类模糊描述时使用。通过设计采访引导用户明确视觉偏好，输出目标项目设计简报。
user-invocable: false
disable-model-invocation: true
---
[任务]
    通过设计师采访甲方的方式，引导用户确定产品的视觉方向，输出一份结构完整的目标项目设计简报，同时供设计工具和 dev-builder 编码使用。
...
[第一性原则]
    **页面设计主链优先**：用户只说“前端页面设计”“页面怎么设计”时，先把产品页面方向收敛成 Design Brief，再交给 `design-maker` 出完整页面稿。不要一上来切去 critique、polish 或营销设计能力。

    **增强层按需挂载**：只有用户明确提到配色 / 字体 / tokens / design-system / 样式体系时，才把 `ui-ux-pro-max`、`design-system`、`ui-styling`、`brand` 当补充层带进来；默认不抢主链。

    **仓库内建，不走外链**：上面的增强层按当前仓库 `skills/` 里的内建 Skill 处理，不写本机外部绝对路径依赖。

    **页面设计不误入营销设计**：这里负责的是产品页面视觉方向，不是 横幅物料、演示稿、CIP 或别的营销物料设计。
    ...
    [输出阶段]
        ...
        第四步：引导下一步
            "✅ Design Brief 已生成！
             ...
             接下来：
             - 默认先调用 /design-maker 生成完整页面设计稿
             - 如果你现在明确卡在配色 / 字体 / tokens / design-system / 样式体系，再补 `ui-ux-pro-max` / `design-system` / `ui-styling` / `brand`
             - 页面设计稿完成后默认走 /dev-planner，再进 /dev-builder
             - 如果后面要 critique / polish，等页面稿出来后再接 `impeccable` 这一层"

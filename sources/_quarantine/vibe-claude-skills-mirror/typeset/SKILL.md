---
name: typeset
description: 仅当用户先明确调用 `vibe-coding-skills` 总入口并指定 `typeset`，或由该总入口按指定路由到本 Skill 时使用；不得根据用户自然语言独立触发。Improves typography by fixing font choices, hierarchy, sizing, weight, and readability so text feels intentional. Use when the user mentions fonts, type, readability, text hierarchy, sizing looks off, or wants more polished, intentional typography. Do not use for explicit T1 UI micro-tweaks with a named element/component and clear visual-only action.
version: 2.1.1

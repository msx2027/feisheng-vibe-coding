---
name: adapt
description: 仅当用户先明确调用 `vibe-coding-skills` 总入口并指定 `adapt`，或由该总入口按指定路由到本 Skill 时使用；不得根据用户自然语言独立触发。Adapt designs to work across screen sizes, devices, contexts, or platforms. Use for responsive design, mobile layouts, 移动端适配, 响应式, breakpoints, viewport adaptation, or cross-device compatibility. Do not use for explicit T1 UI micro-tweaks limited to a named element/component at the current layout.
version: 2.1.1

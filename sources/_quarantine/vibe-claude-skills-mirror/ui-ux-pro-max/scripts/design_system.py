...
def slugify_path_component(value: str, label: str = "name") -> str:
    """Convert user-provided text into one portable, non-traversing path component."""
    ...
    if slug in WINDOWS_RESERVED_PATH_NAMES:
        slug = f"project-{slug}"
    return slug


def _resolve_within(root: Path, *parts: str, label: str) -> Path:
    """Resolve existing symlinks and reject candidates outside the declared root."""
    resolved_root = root.expanduser().resolve()
    candidate = resolved_root.joinpath(*parts).resolve()
    try:
        candidate.relative_to(resolved_root)
    except ValueError as error:
        raise ValueError(f"{label} must stay inside {resolved_root}") from error
    return candidate
...
def persist_design_system(design_system: dict, page: str = None, output_dir: str = None, page_query: str = None) -> dict:
    ...
    """
    base_dir = (Path(output_dir) if output_dir else Path.cwd()).expanduser().resolve()
    
    # Use project name for project-specific folder
    project_name = design_system.get("project_name") or "default"
    project_slug = slugify_path_component(project_name, "project name")
    
    design_system_root = _resolve_within(base_dir, "design-system", label="design system root")
    design_system_dir = _resolve_within(design_system_root, project_slug, label="project output")
    pages_dir = _resolve_within(design_system_dir, "pages", label="pages output")
    ...
    master_file = _resolve_within(design_system_dir, "MASTER.md", label="master output")
    ...
    if page:
        page_slug = slugify_path_component(page, "page name")
        page_file = _resolve_within(pages_dir, f"{page_slug}.md", label="page output")

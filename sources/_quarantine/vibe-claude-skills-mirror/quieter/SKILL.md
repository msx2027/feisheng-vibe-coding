---
name: quieter
description: 仅当用户先明确调用 `vibe-coding-skills` 总入口并指定 `quieter`，或由该总入口按指定路由到本 Skill 时使用；不得根据用户自然语言独立触发。Tones down visually aggressive or overstimulating designs while preserving quality. Use when the user mentions 更克制, 太花, too bold, too loud, overwhelming, aggressive, garish, or wants a calmer aesthetic. Do not use for explicit T1 UI micro-tweaks with a named element/component and clear visual-only action.
version: 2.1.1

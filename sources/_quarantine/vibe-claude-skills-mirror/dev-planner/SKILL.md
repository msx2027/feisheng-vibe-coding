---
name: dev-planner
description: 仅当用户先明确调用 `vibe-coding-skills` 总入口并指定 `dev-planner`，或由该总入口按指定路由到本 Skill 时使用；不得根据用户自然语言独立触发。当目标项目需求文档已完成、需要规划怎么分阶段开发时使用。也在 Spec 变更后用于更新已有开发计划。输出目标项目开发计划。
...
[依赖检测]
    ...
    - codebase-memory-scout → 已有项目规划、迭代计划或跨包影响面不清时，可先做代码图侦察；无代码或新项目空白规划不启用
...
[第一性原则]
    ...
    **已有代码侦察原则**：迭代已有项目时，如果目录结构、模块边界、调用链或测试分布会影响 Phase 拆分，先用 `codebase-memory-scout` 获取 packages、hotspots、候选模块和相关测试；新项目从零规划不启用代码图侦察。

---
name: layout
description: 仅当用户先明确调用 `vibe-coding-skills` 总入口并指定 `layout`，或由该总入口按指定路由到本 Skill 时使用；不得根据用户自然语言独立触发。Improve layout, spacing, and visual rhythm for broad composition work. Use for layout feeling off, spacing systems, visual hierarchy, crowded UI, alignment problems, or better composition. Do not use for explicit T1 UI micro-tweaks with a named element/component and clear visual-only action.
version: 2.1.1

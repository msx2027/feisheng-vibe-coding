---
name: polish
description: 仅当用户先明确调用 `vibe-coding-skills` 总入口并指定 `polish`，或由该总入口按指定路由到本 Skill 时使用；不得根据用户自然语言独立触发。Performs a final quality pass for alignment, spacing, consistency, and micro-detail issues before shipping. Use for explicit polish, 精修打磨, 交付前一致性, 细节 QA, finishing touches, or pre-launch review. Do not use for explicit T1 UI micro-tweaks with a named element/component and clear visual-only action.
version: 2.1.1
...
[Vibe Coding Adapter]
    ...
    - UI 精修、复刻、image-to-code、截图 / 设计稿还原或视觉 polish 时，若项目没有对应功能真源，只做呈现层和必要纯 UI 状态，不补业务逻辑；占位范围在交付中说明。
...
**CRITICAL**: Polish is usually the last step, not the first. Don't polish work that's not functionally complete unless the user explicitly asks for visual polish, UI replication, screenshot/design restoration, or image-to-code.

If the user explicitly requests visual polish, UI replication, screenshot/design restoration, or image-to-code on an incomplete feature, do not complete missing business functionality during polish. Preserve placeholders, mock data, disabled/noop states, TODOs, or static UI states until the user explicitly asks for the corresponding feature development, and report those placeholders at handoff.
...
- [ ] Code is clean (no unexplained TODOs, console.logs, commented code; intentional UI placeholders are documented)

**IMPORTANT**: Polish is about details. Zoom in. Squint at it. Use it yourself. The little things add up.

**NEVER**:
- Polish before it's functionally complete, unless the request is explicitly visual polish, UI replication, screenshot/design restoration, or image-to-code

---
name: animate
description: 仅当用户先明确调用 `vibe-coding-skills` 总入口并指定 `animate`，或由该总入口按指定路由到本 Skill 时使用；不得根据用户自然语言独立触发。Review a feature and enhance it with purposeful motion. Use when the user mentions animation, 微交互, 过渡, motion, transition, micro-interaction, hover effects, or making the UI feel more alive. Do not use for explicit T1 UI micro-tweaks with a named element/component and clear visual-only action.
version: 2.1.1
